...
async function initMySqlConnection(): Promise<boolean> {
  try {
    ...
    const [sopRows]: any = await dbPool.query(
    ...
    mySqlLastError = null;
    console.log(
      `[MySQL Laragon] Terhubung ke database "${MYSQL_CONFIG.database}" di ${MYSQL_CONFIG.host}:${MYSQL_CONFIG.port}`,
    );
    return true;
  } catch (error: any) {
    isMySqlConnected = false;
    mySqlLastError = error.message || "Koneksi gagal";
    console.warn(
      `[MySQL Laragon Info] Belum terhubung ke ${MYSQL_CONFIG.host}:${MYSQL_CONFIG.port}: ${mySqlLastError}. Menggunakan fallback runtime storage.`,
    );
    return false;
  }
}
...
app.get("/api/database/status", async (req, res) => {
  ...
  if (isMySqlConnected && dbPool) {
    try {
      const [uRows]: any = await dbPool.query(
        "SELECT COUNT(*) as count FROM `users`",
      );
      ...
      const [cRows]: any = await dbPool.query(
        "SELECT COUNT(*) as count FROM `data_changes`",
      );
    ...
    tables: [
      {
        ...
        description: "Daftar Whitelist Email Terotorisasi & Status Registrasi",
        recordCount: authEmailCount,
      },
      {
        name: "users",
        description:
          "Kredensial Akun Login Aktif (Email, Password, NIP, Peran)",
...
app.get("/api/auth/check-whitelist/:email", async (req, res) => {
  ...
  if (!match) {
    const memFound = memoryAuthorizedEmails.find(
...
app.post("/api/auth/register-password", async (req, res) => {
  ...
  if (!authorized) {
    authorized = memoryAuthorizedEmails.find(
  ...
  const fullName =
    authorized.full_name || authorized.fullName || "Pegawai GTK Terdaftar";
  const nip = authorized.nip || "-";
  const roleTitle =
    authorized.role_title ||
    authorized.roleTitle ||
    "Pelaksana / Pengelola SOP AP";
  ...
  const existingUserIdx = memoryUsers.findIndex(
  ...
  const authIdx = memoryAuthorizedEmails.findIndex(
    (a) => a.email.toLowerCase() === cleanEmail,
  );
  if (authIdx >= 0) {
...
app.post("/api/auth/authorized-emails", async (req, res) => {
  ...
  if (!cleanEmail || !fullName) {
    return res
      .status(400)
      .json({ error: "Email dan nama lengkap pegawai wajib diisi." });
  }
    ...
    try {
      await dbPool.query(
        ...
        [
          ...
          fullName,
          nip || "-",
          roleTitle || "Pegawai / Operator GTK",
        ],
      );
    } catch (err: any) {
  ...
  const existIdx = memoryAuthorizedEmails.findIndex(
    (a) => a.email.toLowerCase() === cleanEmail,
  );
  if (existIdx >= 0) {
...
app.delete("/api/auth/authorized-emails/:email", async (req, res) => {
  ...
  if (isMySqlConnected && dbPool) {
    try {
      ...
      await dbPool.query("DELETE FROM `users` WHERE LOWER(`email`) = ?", [
        targetEmail,
      ]);
    } catch (err) {
  ...
  memoryAuthorizedEmails = memoryAuthorizedEmails.filter(
    (a) => a.email.toLowerCase() !== targetEmail,
  );
  ...
  res.json({
    success: true,
    message: `Izin akses email ${targetEmail} telah dicabut.`,
  });
});
...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (!isAuthorized) {
    const memAuth = memoryAuthorizedEmails.find(
      (a) => a.email.toLowerCase() === cleanEmail,
    );
    if (memAuth) {
      isAuthorized = true;
...
app.get("/api/auth/users", async (req, res) => {
  if (isMySqlConnected && dbPool) {
    try {
      const [rows]: any = await dbPool.query(
        "SELECT `email`, `password`, `full_name` as `fullName`, `nip`, `role_title` as `roleTitle` FROM `users` ORDER BY `id` ASC",
      );
      return res.json(rows);
    } catch (e) {
...
app.post("/api/auth/users", async (req, res) => {
  const { email, password, fullName, nip, roleTitle, currentActorEmail } =
    req.body;
  const cleanEmail = (email || "").trim().toLowerCase();
  ...
  if (isMySqlConnected && dbPool) {
    try {
      ...
      return res
        .status(400)
        .json({ error: "Email sudah terdaftar di MySQL: " + err.message });
    }
  }

  const existingIdx = memoryUsers.findIndex(
    (u) => u.email.toLowerCase() === cleanEmail,
  );
  const newUser = {
    email: cleanEmail,
...
app.put("/api/auth/users/:email", async (req, res) => {
  ...
  const userIdx = memoryUsers.findIndex(
    (u) => u.email.toLowerCase() === targetEmail,
  );
  if (userIdx >= 0) {
...
app.get("/api/sop", async (req, res) => {
  if (isMySqlConnected && dbPool) {
    try {
      const [rows]: any = await dbPool.query(
        "SELECT * FROM `sop_documents` ORDER BY `created_at` ASC",
      );
      if (rows.length > 0) {
