...
export const AccountSecurityModal: React.FC<AccountSecurityModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "my-account" | "all-accounts" | "add-account"
  >("my-account");
  ...
  const passValidation = newPassword
      ...
      if (!passValidation.isValid) {
        setErrorMessage(
          passValidation.errorMessage ||
            "Kata sandi baru tidak memenuhi syarat.",
        );
        return;
      }
  ...
  const handleAddNewAccount = async (e: React.FormEvent) => {
    ...
    if (!emailVal.isValid) {
      setErrorMessage(
        emailVal.errorMessage || "Email pengguna baru tidak valid.",
      );
      return;
    }
    ...
    if (!passVal.isValid) {
      setErrorMessage(
        passVal.errorMessage || "Kata sandi pengguna baru minimal 6 karakter.",
      );
      return;
    }
    ...
    await loadAccounts();
    onSuccessToast(
      "Akun Pengguna Ditambahkan",
      `Akun ${newAcc.email} berhasil tersimpan di database.`,
    );
  ...
  const handleDeleteAccount = async (targetEmail: string) => {
    if (accounts.length <= 1) {
      setErrorMessage(
        "Tidak dapat menghapus akun satu-satunya yang tersisa di database!",
      );
      return;
    }
    if (
      !window.confirm(
        `Yakin ingin menghapus akun ${targetEmail} dari database sistem?`,
      )
    ) {
      return;
    }
    ...
    if (success) {
      ...
      onSuccessToast(
        "Akun Dihapus",
        `Akun ${targetEmail} berhasil dihapus dari database.`,
      );
    } else {
