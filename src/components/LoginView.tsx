...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess }) => {
  ...
  const isPasswordsMatch =
    regPassword === regConfirmPassword && regPassword.length >= 6;
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fallback
      const res = performLogin(email, password);
      if (res.success) {
        onLoginSuccess();
      } else {
        setErrorMessage(res.error || "Terjadi kesalahan login.");
      }
    } finally {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (!whitelistStatus?.isAuthorized) {
      setRegisterErrorMessage(
        "Akses Ditolak: Alamat email tidak memiliki izin akses dalam daftar whitelist SIM-SOP GTK Provinsi Gorontalo.",
      );
      return;
    }
    ...
    if (!isPasswordsMatch) {
      setRegisterErrorMessage(
        "Konfirmasi kata sandi tidak cocok atau panjang kurang dari 6 karakter.",
      );
      return;
    }
    ...
    try {
      const result = await registerPasswordForAuthorizedEmail(
        regEmail,
        regPassword,
      );
      if (result.success) {
        ...
        setRegisterErrorMessage(
          result.message || "Gagal mendaftarkan kata sandi.",
        );
      }
    } catch (err: any) {
      setRegisterErrorMessage(
        err.message || "Terjadi kesalahan server saat mendaftarkan kata sandi.",
      );
    } finally {
      ...
      <div className="w-full max-w-xl z-10">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-300 max-w-md mx-auto mt-0.5">
            Sistem Informasi Manajemen Standar Operasional Prosedur Administrasi
            Pemerintahan
          </p>
          ...
          <div className="mt-2.5 inline-flex items-center space-x-1.5 px-3.5 py-1 rounded-full bg-blue-950/80 border border-blue-600/50 text-[11px] text-blue-200 font-semibold shadow-inner">
            ...
            <span>
              Sistem Terproteksi Penuh &bull; Pembatasan Akses Email
              Terotorisasi
            </span>
          </div>
        </div>
              ...
              <>
                <div className="border-b border-slate-100 pb-3 mb-5">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Hanya email yang telah diizinkan dan telah mendaftarkan kata
                    sandi yang dapat membuka sistem.
                  </p>
                </div>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm font-bold text-rose-950 mt-0.5 leading-snug">
                          Akun diblokir karena masih melakukan kesalahan setelah
                          masa pembekuan 1 menit.
                        </div>
                        <p className="text-xs text-rose-700 mt-1.5 leading-relaxed">
                          Sistem telah menonaktifkan formulir login. Silakan
                          gunakan <strong>Layanan Pengaduan Pengembang</strong>{" "}
                          di bawah ini untuk pembukaan blokir akses.
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-2 text-[11px] font-semibold text-rose-700">
                          Peringatan: Jika masih salah 1 kali lagi setelah waktu
                          ini, akun akan diblokir total dan tidak bisa lagi
                          login.
                        </div>
                      </div>
                    </div>
                ...
                <div className="mt-4 text-center">
                  ...
                  >
                    ...
                    <span className="text-emerald-700 underline">
                      Daftar / Aktivasi Sandi di sini &rarr;
                    </span>
                  </button>
                </div>
              ...
              <>
                <div className="border-b border-slate-100 pb-3 mb-5">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Sistem dibatasi secara ketat. Masukkan email resmi yang
                    telah ditentukan untuk mendaftarkan/memperbarui kata sandi
                    akun Anda.
                  </p>
                </div>
                  ...
                  <div className="mb-5 p-4 rounded-xl bg-emerald-50 border-2 border-emerald-500 text-emerald-950 flex items-start space-x-3 text-xs animate-in fade-in duration-300">
                    ...
                    <div>
                      <div className="font-bold text-sm text-emerald-900">
                        Pendaftaran Kata Sandi Berhasil!
                      </div>
                ...
                <form onSubmit={handleRegisterSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-1">
                      Sistem akan memverifikasi apakah email ini tercantum dalam
                      SK/Daftar Pegawai GTK yang disetujui.
                    </p>
                  </div>
                      ...
                      {whitelistStatus.isAuthorized ? (
                        <div className="p-3.5 rounded-xl bg-emerald-50 border border-emerald-300 text-emerald-950 text-xs space-y-1.5 shadow-xs">
                          <div className="flex items-center space-x-2 font-bold text-emerald-900">
                            ...
                            <span>
                              Email Terverifikasi &amp; Memiliki Izin Akses
                            </span>
                          </div>
                          <div className="bg-white/80 rounded-lg p-2.5 border border-emerald-200 text-[11px] text-slate-700 space-y-0.5">
                            <div>
                              ...
                              <strong className="text-slate-900">
                                {whitelistStatus.authorizedAccount?.fullName}
                              </strong>
                            </div>
                            <div>
                              NIP:{" "}
                              <span className="font-mono text-slate-900 font-semibold">
                                {whitelistStatus.authorizedAccount?.nip || "-"}
                              </span>
                            </div>
                            <div>
                              Jabatan:{" "}
                              <span className="text-slate-900 font-medium">
                                {whitelistStatus.authorizedAccount?.roleTitle}
                              </span>
                            </div>
                        ...
                        <div className="p-3.5 rounded-xl bg-rose-50 border border-rose-300 text-rose-950 text-xs flex items-start space-x-2.5 shadow-xs">
                          ...
                          <div>
                            <div className="font-bold text-rose-900">
                              Akses Ditolak &bull; Email Tidak Terotorisasi
                            </div>
                            ...
                            <div className="text-[10px] text-rose-700 mt-1">
                              Silakan gunakan alamat email dinas yang telah
                              didaftarkan Administrator (lihat daftar di bawah).
                            </div>
                          </div>
                        </div>
                      )}
        ...
        {shouldShowHelpdesk && (
          <div className="mt-6 rounded-2xl border p-4 sm:p-5 transition-all shadow-xl bg-rose-950/90 border-rose-500 text-white ring-2 ring-rose-400/50 animate-in fade-in slide-in-from-bottom-3 duration-300">
            ...
            <p className="text-[11px] text-rose-100 mt-2.5 leading-relaxed">
              Anda tidak dapat melakukan login lagi karena telah melebihi batas
              maksimal percobaan. Silakan hubungi layanan pengaduan pengembang
              untuk memverifikasi identitas dan membuka kembali akun Anda:
            </p>
        ...
        <div className="text-center mt-6 text-slate-400 text-xs">
          <div>
            Hak Cipta &copy; 2026 Kantor Guru dan Tenaga Kependidikan Provinsi
            Gorontalo
          </div>
